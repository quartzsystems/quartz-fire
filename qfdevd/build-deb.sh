#!/usr/bin/env bash
# Build qfdevd_*.deb inside a Debian bookworm container with a current Rust
...
# ext4 — only the finished .deb is copied back to the host.
set -euo pipefail
...
echo "==> Building qfdevd .deb in rust:1-bookworm"
docker run --rm \
  -v "$repo_root/qfdevd":/src-qfdevd:ro \
  -v "$repo_root/packages":/out \
  rust:1-bookworm bash -euo pipefail -c '
  export DEBIAN_FRONTEND=noninteractive
  apt-get update -qq
  # build-essential supplies the C compiler rusqlite (bundled SQLite) needs.
...
ls -1 "$repo_root"/packages/qfdevd_*.deb
