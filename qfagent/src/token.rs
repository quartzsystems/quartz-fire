...
//! naming that segment, so a mangled copy-paste fails the commit with a
//! message the operator can act on. The parsed [`EnrollToken`] never prints
//! its secret (`Debug` redacts it) — the raw token string must not be logged.
...
pub struct EnrollToken {
    ...
    pub token_id: String,
    pub secret: String,
    /// SHA-256 of the controller's issuing CA certificate (DER).
    pub ca_fingerprint: [u8; 32],
    /// SHA-256 of the full token string — the commit idempotency key (a
    /// config.boot replay with an already-consumed token must not re-enroll).
...
pub enum TokenError {
    ...
    TooLong(usize),
    #[error("enrollment token contains whitespace or control characters — paste it as a single unbroken line")]
    BadCharacters,
    #[error("enrollment token must have 5 '|'-separated segments (version|gateway|org|token|ca-fingerprint), found {0}")]
    SegmentCount(usize),
    #[error("segment 1 (version) must be 'QC1', found '{0}' — this firmware only understands QC1 tokens")]
    Version(String),
