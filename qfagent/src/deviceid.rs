...
//! of four (`QF-XXXX-XXXX-XXXX-XXXX`). This is a verbatim port of the
//! QuartzCommand server's `backend/src/pki/deviceid.rs`; enrollment verifies
//! the device's claimed ID against the server's own derivation, so the two
//! implementations must never drift — the fixed vectors below pin ours.
...
mod tests {
    ...
    /// 16-char truncation, and grouping are all load-bearing wire format
    /// (the server derives the same string and rejects mismatches).
    /// Cross-checked against an independent implementation, not this one.
    #[test]
    fn fixed_vectors() {
        // 32 zero bytes.
        let zero = [0u8; 32];
        ...
        assert_eq!(derive_device_id(&zero), "QF-CSM7-NBFR-CAYQ-EV4F");
        assert_eq!(derive_device_id(&seq), "QF-NRGP-RBQN-4HX3-F0P1");
        assert_eq!(derive_device_id(&rfc8032), "QF-47Z3-3QX1-AJH6-2RKB");
    }
